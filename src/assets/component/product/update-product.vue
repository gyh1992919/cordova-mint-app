<template>
    <div class="add-product">
        ...
        <div class="content">
            <mt-field label="标题" placeholder="最多可输入48个字符" v-model="updateProduct.name" :attr="{ maxlength: 48 }"></mt-field>
            <mt-field  placeholder="简单描述一下您的商品/服务，字数限制60字符" type="textarea" v-model="updateProduct.content" rows="5" :attr="{ maxlength: 60 }" ></mt-field>
            <div  class="imglist" v-if="updateProduct">
                <div class="imgitem"  v-for="(itemImg,index) in productImage"  :key="index">
                    <img :src="itemImg.url" alt="" class="imgurl">
                    <div  class="imgtitle">{{imgTitle[index]}}</div>
                    <div class="close" @click="removeImage(index,itemImg)">
                        <div  class="icon">x</div>
                    </div>
                </div>

                <div class="imgitem"  v-for="(itemImg,index) in updateProduct.contentImages"  :key="index">
                    <img :src="itemImg.url" alt="" class="imgurl">
                    <div  class="imgtitle">{{imgTitle_2[index]}}</div>
                    <div class="close"  @click="removeContentImage(index)">
                        <div  class="icon">x</div>
                    </div>
                </div>
            ...
            <mt-field label="价格：" type="number" v-model="updateProduct.price"></mt-field>
            <mt-cell title="分类" :value=" categValue ?  categValue : updateProduct.categoryId" isLink @click.native="categoriePopup = true" ></mt-cell>
        </div>

        <mt-popup  v-model="categoriePopup" position="right" class="categoriePopup"  :modal="false">
            <mt-button @click.native="closeCategoriePopup" size="large" type="primary">确定返回</mt-button>
            <mt-radio title="只可选择一个" v-model="categValue" :options="categoriesList"  @change="changeValue">
            </mt-radio>
        </mt-popup>
        <div class="addbtn">
            <mt-button size="large" type="primary"  @click.native="getCreateProduct" :disabled="updateProduct.images.length < 3">确认更新</mt-button>
        </div>
...
<script>
    ...
    export default {
        data () {
            return  {
                updateProduct: this.$store.state.UpdateProduct.data,
                popupVisible1: false,
                productImage: [],
                images: [],
                ...
                categValueId: '',
                imgTitle: ["主图","正面","背面"],
                imgTitle_2: ["详情图","详情图","详情图","详情图","详情图"]
            }
        },
        mounted() {
            this.buttonBottom = this.$refs.button.offsetTop;
            console.log( this.$store.state.UpdateProduct.data.images);
            this.productImage = this.$store.state.UpdateProduct.data.images;
            this.getCategories();
        },
        methods: {
            addProductImg () {
                ...
                function onLoadImageUploadSuccess(imageURI) {
                    ...
                    ft.upload(imageURI, encodeURI('http://test.mihutime.com/employee/v2/files'), function (r) {
                        ...
                        if(self.productImage.length>2) {
                            self.updateProduct.contentImages.push({id: resObject.id, url:resObject.url})
                            self.$nextTick()
                        } else {
                            self.productImage.push({id: resObject.id, url:resObject.url})
                            self.images.push({id: resObject.id, url:resObject.url})
                            self.$nextTick()
                        }
                    }, function (error) {
            ...
            removeImage (index,img) {
                this.$http({url:'/products/'+this.updateProduct.id +'/images/'+img.id, method:'DELETE'}).then(res =>{
                    console.log(res.data)
                })
                this.productImage.splice(index, 1)
            },
            removeContentImage (index) {
                this.updateProduct.contentImages.splice(index, 1)
            },
            changeValue (value) {
            ...
            getCreateProduct() {
                this.$http({
                    url:'/products/'+this.updateProduct.id,method: 'PUT',
                    data: {
                        id:this.updateProduct.id,
                        name: this.updateProduct.name,
                        content: this.updateProduct.content,
                        price: this.updateProduct.price,
                        newImages:  this.images,
                        contentImages: this.updateProduct.contentImages,
                        categoryId: this.categValueId,
                    }
                }).then( res=> {
                    navigator.notification.alert('更新成功', null, "提醒");
                    this.$router.back()
                }, error => {
